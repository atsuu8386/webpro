...
<div class="page page-center">
    <div class="container container-tight py-4">
        <div class="card card-md">
            <div class="card-body">
                <h2 class="h2 text-center mb-4">{{ __('auth.reset_password_title') }}</h2>
                <p class="text-secondary mb-4">{{ __('auth.reset_password_description') }}</p>
                ...
                <form method="POST" action="{{ route('password.store') }}" autocomplete="off">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="password" class="form-control @error('password') is-invalid @enderror"
                            id="password" name="password" placeholder="{{ __('auth.new_password_placeholder') }}" required>
                        @error('password')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-footer">
                        <button type="submit" class="btn btn-primary w-100">{{ __('auth.reset_password_button') }}</button>
                    </div>
                </form>
            </div>
        </div>
